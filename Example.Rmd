...
#Get ipsl monthly inputs for 3b - file below is called pp, just read in first row of historical
 ...
 inputs$year<-rep(1850:2014,each= 12)
...
inputs$slope=GetPPIntSlope(inputs$sphy,inputs$lphy,mmin=10^-14.25, mmid=10^-10.184,mmax=10^-5.25,depth=2776.039,output="slope")

inputs$intercept=GetPPIntSlope(inputs$sphy,inputs$lphy,mmin=10^-14.25, mmid=10^-10.184,mmax=10^-5.25,depth=2776.039,output="intercept")

inputs$er=getExportRatio(inputs$sphy,inputs$lphy,depth=2776.039,sst=inputs$sst)
...
# get annual means
# first year for spin up
inputs_yr1<-inputs %>% filter(year==1850) %>% summarise_at(vars(lon:depth),mean)
# repeat to keep create burn in of 100 yrs* time steps per year
inputs_yr1<-as.data.frame(lapply(inputs_yr, rep, 12*100))

#annual means
inputs_yr<-inputs%>% 
group_by(year) %>% 
  mutate(across(lphy:depth, mean)) %>%
  ungroup

inputs_yr<-inputs_yr1


# revert back to full time series
# inputs_yr<-inputs - need to smooth these or add a burn.in
...
params <- sizeparam(equilibrium = FALSE
                        ...
                        ,depth = unique(inputs_yr$depth)
                        #,er = inputs$ts$er
                        ,er = inputs_yr$er
                        #,pp = inputs$ts$intercept
                         ,pp = inputs_yr$intercept
                        #,slope = inputs$ts$slope
                         ,slope = inputs_yr$slope
                        ,sst = inputs_yr$sst
                        ,sft = inputs_yr$sbt
                        ,lat = unique(inputs_yr$lat)
                        ,lon = unique(inputs_yr$lon)
                        ,use.init = FALSE
                        ,U.initial = NA 
                        ,V.initial = NA  
                        ,W.initial = NA) 
